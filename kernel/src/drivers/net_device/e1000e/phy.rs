...
pub fn init_phy(dev: &mut E1000eDevice) -> Result<(), ErrorCode> {
    ...
    mdio::write(&mdic_reg, dev.phy_addr, 0, 1 << 15)?; //reset
    while mdio::read(&mdic_reg, dev.phy_addr, 0)? & (1 << 5) != 0 {}
    ...
    let mut status = mdio::read(&mdic_reg, dev.phy_addr, 1)?;
    while (status >> 2) & 1 == 0 && new - start < Duration::from_millis(1) {
        status = mdio::read(&mdic_reg, dev.phy_addr, 1)?;
        new = std::time::Instant::now();
    }
    dev.link_up = (status >> 2) & 1 == 1;
