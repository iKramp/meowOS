...
use crate::{
    drivers::filesystem::rfs2::{Rfs2, operations::DirEntry},
    memory::physical_allocator,
};

impl Rfs2 {
    //contains an additional allocation for linking
    pub(super) async fn read_direntries(&self, dir_root: u64) -> (PhysAddr, &'static mut [DirEntry]) {
        ...
        let buf_vec = (0..size_pages - 1).map(|i| buf + i * 4096).collect::<Vec<_>>();
        self.read_locked(dir_root, 0, (size_pages - 1) * 4096, &buf_vec)
            .await
            .expect("correct args");
    ...
    pub(super) async fn write_direntries(&self, dir_root: u64, buffer: PhysAddr, num_entries: usize) {
