...
pub fn get_device_id(dev: &PciDevice) -> u16 {
    (get_dword(0, dev) >> 16) as u16
}

pub fn get_subsystem_vendor_id(dev: &PciDevice) -> u16 {
    get_dword(0x2C, dev) as u16
}

pub fn get_subsystem_id(dev: &PciDevice) -> u16 {
    (get_dword(0x2C, dev) >> 16) as u16
}

pub fn get_command(dev: &PciDevice) -> u16 {
    get_dword(4, dev) as u16
}

pub fn set_command(value: u16, dev: &PciDevice) {
    set_dword(4, value as u32, dev);
}

pub fn get_status(dev: &PciDevice) -> u16 {
    (get_dword(4, dev) >> 16) as u16
}

pub fn get_revision_id(dev: &PciDevice) -> u8 {
    get_dword(8, dev) as u8
}

pub fn get_progif(dev: &PciDevice) -> u8 {
    (get_dword(8, dev) >> 8) as u8
}

pub fn get_class(dev: &PciDevice) -> PciClass {
...
pub fn get_bist(dev: &PciDevice) -> u8 {
    (get_dword(0xC, dev) >> 24) as u8
}

pub fn get_latency_timer(dev: &PciDevice) -> u8 {
    (get_dword(0xC, dev) >> 8) as u8
}

pub fn get_cache_line_size(dev: &PciDevice) -> u8 {
    get_dword(0xC, dev) as u8
}

pub fn get_bar(index: u8, dev: &PciDevice) -> Option<(Bar, u8)> {
...
fn get_bar_size(index: u8, mask: u32, dev: &PciDevice) -> u32 {
...
fn get_64b_mem_bar_size(index: u8, dev: &PciDevice) -> u64 {
...
pub fn get_capabilities_pointer(dev: &PciDevice) -> u8 {
    (get_dword(0x34, dev) & 0b11111100) as u8
}

pub fn load_capabilities_list(dev: &mut LegacyPciDevice) {
    let status = get_status(&dev.device);
    if (status & 0x10) == 0 {
        return;
    }
    let mut capabilities = Vec::new();
    let mut pointer = get_capabilities_pointer(&dev.device);
    while pointer != 0 {
        let capability_first_dword = get_dword(pointer, &dev.device);
