use std::{alloc::borrow::ToOwned, boxed::Box, error::ErrorCode, vec::Vec};
...
async fn cmd_cp_internal(mut args: CommandSplitter) -> Result<(), ErrorCode> {
    let source_path = args.next().ok_or(ErrorCode::InvalidArgument)?;
    let dest_path = args.next().ok_or(ErrorCode::InvalidArgument)?;
...
async fn copy_dir(
    ...
    source_path.push(name.to_owned().into_boxed_str());
    ...
    for entry in direntries {
        ...
        let entry_file = vfs::open_file(
            resolved_src_path.index(0..resolved_src_path.iter().count() + 1),
            None,
            *vfs::file::OpenFlags(0).set_read(true),
        )
        .await?;
